...
STOPWORDS = {"de", "la", "el", "en", "y", "que", "los", "las", "un", "una", "es", "por", "para", "con", "del", "se", "como", "mi", "me", "hay"}

def compute_metrics(messages: list[dict]) -> dict:
    sessions = {m["session_id"] for m in messages}
    user_msgs = [m for m in messages if m["role"] == "user"]
    words = [w.lower() for m in user_msgs for w in m["content"].split()
    ...
    return {
        ...
        "avg_user_msg_length": round(sum(len(m["content"]) for m in user_msgs) / len(user_msgs), 1) if user_msgs else 0,
